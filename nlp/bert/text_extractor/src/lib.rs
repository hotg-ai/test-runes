...
mod tests {
    ...
    fn test_token_extractor() {
        ...
        let end_index = Tensor::new_vector(vec![5]);

        let mut text_extractor = TextExtractor::default();
        let out = text_extractor.transform((bytes, start_index, end_index));

        let output = Tensor::new_vector(vec![Cow::Borrowed("unaffable")]);
